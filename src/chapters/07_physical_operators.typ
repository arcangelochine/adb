...
#let abs = [
  ...
  memory and minimal disk access. This chapter develops the machinery behind
  that process. It begins with the iterator model—the execution protocol that
  allows an entire operator tree to run efficiently by producing one record at a
  time—and the access method interface through which operators interact with
  data structures on disk. It then examines each unary physical operator in
  turn: table and index scans, projection, duplicate elimination, sorting, and
  grouping. For each operator the chapter establishes its cost, its memory
  requirements, its blocking behavior, and the conditions under which the output
  is guaranteed to be sorted or grouped—properties that propagate upward through
  the tree and determine whether subsequent operators must pay for an explicit
  sort. The chapter closes with the problem of estimating the size of an
  operator's output, which is the prerequisite to every cost comparison the
  optimizer must make.
]
...
#chapter(
  ...
  The path from a SQL statement to a stream of result records passes through
  four transformations. Parsing converts the text into an internal
  representation and checks types. Logical planning produces a relational
  algebra expression that is semantically equivalent to the query—a `FROM`
  clause becomes a join, a `WHERE` clause becomes a selection, a `SELECT` clause
  becomes a projection. Logical optimization applies algebraic identities to
  improve the plan, most importantly pushing selections as close as possible to
  the leaves so that early filtering reduces the size of intermediate results.
  Finally, physical planning replaces every logical operator with a physical
  one: a specific algorithm with a specific cost. A logical join becomes an
  index nested loop, a merge join, or a hash join. A logical selection becomes a
  table scan followed by a filter, or an index filter that reads directly from
  the index. The component responsible for physical planning is the _optimizer_,
  and its choices determine query performance more than any other factor in the
  system.

  Physical plans are represented as trees rather than linear expressions,
  because linear notation becomes unreadable as soon as binary operators such as
  join appear. In the tree representation, leaves are operators that read data
  from disk—table scans, index filters—and internal nodes are operators that
  transform data received from their children. Data flows upward from leaves to
  root; control flows downward from root to leaves through requests for the next
  record. This asymmetry is the foundation of the iterator model.

  == The Iterator Model

  The naive approach to executing an operator tree is _operator-at-a-time_:
  evaluate the leftmost leaf completely, store its result on disk, evaluate the
  next operator on that stored result, store again, and so on until the root is
  reached. With five operators in the tree, five intermediate relations are
  materialized, each requiring a full write and a full read. This is precisely
  the pattern that the previous chapters established as the dominant source of
  cost in disk-based systems.

  The alternative is _tuple-at-a-time_ or _iterator_ execution. Every physical
  operator implements three methods. Open initializes the operator and
  recursively opens its children. Next produces the next output record: the
  operator requests records from its children, applies its logic, and either
  returns a record to its parent or requests another from its children. Close
  releases all resources. When the root of the plan tree is asked for its next
  record, it asks its children, which ask their children, and so on down to the
  leaves. The leaves are the only operators that actually read from disk. Every
  other operator receives records one at a time from its children, processes
  them in memory, and passes results upward.

  // FIGURE SUGGESTION:
  // A tree diagram of a small physical plan—projection over a filter over a table scan,
  // with an index nested loop join as a sibling subtree—with downward arrows labeled
  // "next()" and upward arrows labeled "record / EOF". Annotate the leaf nodes as the
  // only points of disk I/O. This is the central figure of the iterator model section
  // and should appear immediately after this paragraph.

  The memory advantage is striking. A filter operator holds one record in memory
  at a time: it requests a record from its child, tests the predicate, and
  either returns the record or discards it and requests another. A projection
  holds one record. A nested loop join holds one record from its outer input and
  one from its inner input simultaneously. The total memory consumed by all
  non-leaf, non-blocking operators is bounded by the number of operators
  multiplied by the size of one record—negligible compared to the data.

  There are exceptions. Sorting must see all records before producing any
  output. Hash-based operations—`HashJoin`, `HashDistinct`, `HashGroupBy`—must
  build an in-memory hash structure before the probe phase begins. These
  operators are _blocking_: the first call to next exhausts their entire input
  before returning a single record to the parent. They are also the only
  operators that access disk for intermediate storage. All other operators are
  non-blocking and touch disk only indirectly through their leaf children.

  The cost of the iterator model is implementation complexity. Each operator
  must preserve its state across successive calls to next. A nested loop join
  must remember which record it holds from the outer relation and where it is in
  the scan of the inner relation. This bookkeeping is straightforward but
  requires care. It is also the reason why all operators in the tree consume a
  small amount of memory even before any data is read, because all open
  operators maintain their state simultaneously. In practice this overhead is
  negligible compared to the data volume.

  === The Access Method Interface

  The iterator model requires a uniform interface through which operators
  request records regardless of the underlying data structure. This interface is
  provided by the _access method layer_, which wraps every data structure—heap
  file, B+-tree index, hash table—behind a common cursor abstraction.

  A _file scan cursor_ on a heap file supports `open`, which positions the
  cursor before the first record; `isDone`, which signals exhaustion;
  `getCurrent`, which returns the record at the current position; and `next`,
  which advances the cursor. A simplified variant collapses `getCurrent` and
  `next` into a single call that returns the current record and advances, or
  returns a sentinel end-of-file value. An _index scan cursor_ extends the file
  scan with a range: when opened, it receives a first key and a last key and
  scans only the portion of the index between those bounds. Equality search is
  the special case where first key equals last key. A full index scan provides
  minus infinity and plus infinity as bounds.

  This uniform interface means that a filter operator works identically whether
  its input is a heap file or a B+-tree index. The decision of which data
  structure to use is made once, at plan construction time, by the optimizer. At
  execution time, operators call open, isDone, getCurrent, and next without
  knowing what lies beneath.

  When an index is used to retrieve records from a heap file, two cursors are
  active simultaneously: one on the index, which produces RIDs, and one on the
  heap file, which maps each RID to a page and record. This two-level access
  corresponds directly to the $C_I + C_D$ cost model.
  ...
  `(name,
  age)` unless the functional dependency `name -> age` holds. Functional
  ...
  selectivity factor of the predicate. The output preserves the order and
