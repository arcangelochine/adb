...
#chapter(
  ...
  the underlying data structure---heap file, B+-tree, hash table---was developed
  in the previous chapter, along with the $C_I + C_D$ cost model for two-level
  ...
  works on any primary organization---heap, sequential, hash, or
  B+-tree---because every organization supports a sequential scan. Unless
  otherwise specified, all tables in cost analyses are assumed to be stored as
  heap files.
  ...
  two-phase case---`IndexScan` is _rarely competitive_. The one exception is
  when the index is clustered, in which case the RIDs are already in page order
  and the effective cost is $N_"PAG"$ with the data arriving pre-sorted. Outside
  of this case, `IndexScan` is not used.
  ...
  then accesses the data file once per surviving record---or more precisely,
  once per page containing at least one surviving record, as estimated by the
  ...
  later one---allow attributes to be freely inserted or removed from the sort
  key without affecting the grouping guarantee. Functional dependencies that go
  ...
  its child through the iterator interface---applies a hash function $h_1$ to
  the deduplication attributes, and writes each record to one of $B$ output
  ...
  the last one read---and it means that `Sort` imposes a latency equal to its
  full execution time before any result is visible to the parent operator.
  `Sort` also reserves $B$ buffer pages for the duration of its execution, from
  `open` until the final `next` returns end-of-file (`EOF`).
  ...
  passes---$ceil(log_B N_"PAG")$---for the same input size, and both have the
  same incremental cost of $2 times N_"PAG"$ in the standard two-phase case. The
  ...
  grouped on the dimension attributes---once a particular combination of
  dimension values disappears from the stream, it must never reappear.
  `Distinct` is in fact a special case of `GroupBy` where no aggregate functions
  are computed.
  ...
  between `Sort` and `HashDistinct`. When the data arrives pre-sorted---because
  it came from an index, from an earlier sort, or from a join that preserves
  ...
  must occur---otherwise the user would not have written `Distinct`---while
  avoiding the absurdity of estimating more output records than input records.
  The factor of two is an empirical convention rather than a derived quantity:
  it encodes the belief that a `Distinct` operation eliminates at least half the
  records.
  ...
  total plan cost is approximately $1.010$ I/Os---better than table scan
  precisely because the selectivity factor is $0.1%$, below the crossover point
  of roughly $1%$ at which table scan becomes competitive.
