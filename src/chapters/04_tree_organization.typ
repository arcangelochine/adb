...
#chapter(title: "Tree Organization", abstract: abs, toc: true)[
  ...
  Third, awareness of the memory hierarchy is crucial. The observation that upper levels of a B+-tree typically fit in main memory transforms the performance analysis, making equality searches cost approximately one disk access regardless of tree height. This insight depends on understanding both the structure's fan-out and the realistic size of main memory in contemporary systems.

  Fourth, probabilistic thinking informs cost estimation. The analysis of split cascades during insertion shows that worst-case behavior occurs with exponentially decreasing probability as the tree grows taller, making the amortized cost of insertion far lower than the worst-case cost would suggest.

  The B+-tree's dominance in practice stems from its successful navigation of these trade-offs. By providing near-optimal performance for both equality and range queries, remaining highly dynamic without requiring reorganization, and achieving sufficient storage efficiency for most applications, the B+-tree has earned its position as the foundational structure for both primary file organizations and secondary indexes in virtually all modern database systems.
