...
#chapter(title: "Indexes", abstract: abs, toc: true)[
  ...
  the number of records per page---the cost of chasing individual RIDs through
  the heap exceeds the cost of a plain table scan. For a typical page holding
  100 records, this threshold sits near 1%. Range queries selecting more than 1%
  of records should generally not use an unclustered secondary index.
  ...
  file optimizes table scans---it is the most compact possible
  organization---but provides no fast access on any attribute. By pairing a heap
  file with one or more secondary indexes, a system gains _optimal table scan
  performance_ on any attribute alongside fast equality search on indexed
  attributes, at a cost of 2 I/Os rather than 1: one for the index leaf and one
  for the data page. This modest increase from one to two I/Os for equality
  search is almost always an acceptable trade-off given the gains in
  flexibility, which is why heap-plus-index is the default design in virtually
  every relational database system.
  ...
  record in ten thousand---the situation is paradoxical: each individual RID
  list is tiny, but using only one of them to find the data and then filtering
  in memory with the other means reading ten thousand times more data pages than
  ...
  #info-box()[
    ...
    The index traversal cost $C_I = ceil(s_f times N_"LEAF")$ holds---meaning
    the index is genuinely useful---only for predicates where the qualifying
    entries are contiguous in the leaf level. This is the case precisely when
    the predicate has one of the following forms.
  ...
  bit vector of length $N_"REC"$---one bit per record in the table, set to one
  if and only if that record carries the corresponding value. The two
  ...
  grade---these are all good candidates. An attribute like a personal
  identifier, a timestamp, or a continuous measurement is a poor candidate: the
  number of distinct values grows with the table, and the bitmap index grows
  _quadratically_.
  ...
  $v$---the gap before it---plus one additional number for the final gap. The
  total storage is therefore one number per matching record, precisely as in the
  ...
  natural. For two-dimensional spatial data---latitude and longitude, or any
  pair of Cartesian coordinates---a linear order is an imperfect fit, and the
  standard approaches fail in ways that are worth understanding precisely.
  ...
  $(x_2, y)$ for the same $y$---which are nearby in two-dimensional space---may
  be separated by the entire range of $y$ values for all intermediate $x$
  values. In terms of the index, two restaurants that are ten meters apart
  north-south but at slightly different east-west positions can be separated by
  the entire contents of a vertical strip spanning thousands of kilometers. A
  range on $y$ within a range on $x$ touches a non-contiguous set of index
  entries, and the traversal cost approaches $N_"LEAF"$---the cost of reading
  the entire index.
  ...
  structure---no partition tree, no spatial index hierarchy---needs to be stored
  on disk. The partition tree is a mental model for understanding the path
  ...
  false positives---records that lie in a region intersecting the query range
  but not within the rectangle itself.
  ...
  The three approaches to conjunctive queries---separate single-attribute
  indexes with RID-list intersection, multi-attribute combined indexes, and
  spatial indexes such as the G-tree---are not interchangeable. Each is suited
  ...
  contains---and in what proportions---is therefore the prerequisite to choosing
